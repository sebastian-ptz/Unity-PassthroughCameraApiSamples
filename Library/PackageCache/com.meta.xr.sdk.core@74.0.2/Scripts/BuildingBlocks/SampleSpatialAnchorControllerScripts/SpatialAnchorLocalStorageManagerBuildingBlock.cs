...
namespace Meta.XR.BuildingBlocks
{
    ...
    public class SpatialAnchorLocalStorageManagerBuildingBlock : MonoBehaviour
    {
        ...
        {
            Debug.Log($"[{nameof(SpatialAnchorLocalStorageManagerBuildingBlock)}] Trying to save uuid to local storage: {anchor.Uuid}");

            if (result != OVRSpatialAnchor.OperationResult.Success)
            {
                Debug.Log($"OVRSpatialAnchor failed operation for uuid: {anchor.Uuid}");
                return;
            }
